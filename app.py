...
# ==========================================
# ২. ডাটাবেজ সেটআপ ও টেবিল আর্কিটেকচার (আপনার অরিজিনাল স্কিমা)
# ==========================================
...
def init_db():
    ...
    # কর্মচারী টেবিল (৪টি মূল ফটোপ্যাথ সহ সম্পূর্ণ রিয়েল স্কিমা)
    cursor.execute("""
    ...
    # সেকেন্ড পার্টি টেবিল (App 3.5 এর অরিজিনাল কলাম নেম হুবহু লক করা হলো)
    cursor.execute("""
    ...
    # 🎯 লাইন ১২০ এর ক্র্যাশ ফিক্স: আপনার অরিজিনাল লুপ ও কলাম স্ট্রাকচার
    default_parties = ["Digi", "Dss", "Lifting", "Volt Transfer", "Others"]
    for party in default_parties:
        cursor.execute("""
            INSERT OR IGNORE INTO second_parties 
...
if admin_password == "admin786":
    ...
    st.sidebar.success("অ্যাক্সেস মোড: Admin")
else:
    st.session_state.user_role = 'User'
    st.sidebar.info("অ্যাক্সেস মোড: User/Staff")
...
# 🚨 সাইডবার অপশন চেঞ্জ হলে মেইন বডির ওল্ড ডাটা ব্লাইন্ড ফ্লাশ করার মেকানিজম
...
if current_action == "📝 Daily Cash Khata":
    ...
    # সেকেন্ড পার্টি লিস্ট রিট্রিভ (Active স্ট্যাটাস ফিল্টার সহ)
    conn = get_db_connection()
    parties = [r['party_name'] for r in conn.execute("SELECT party_name FROM second_parties WHERE company=? AND status='Active'", (current_company,)).fetchall()]
    conn.close()
    parties = [""] + parties
...
# ------------------------------------------
# ঘ) कर्मचारी ডিরেক্টরি (Employee Directory)
# ------------------------------------------
    ...
    if excel_file:
        ...
        if st.button("🚀 ডাটাবেজে মার্চ করুন (Sync Database)"):
            ...
            conn.commit()
            conn.close()
            st.success("বাল্ক ফাইল সফলভাবে মার্চ করা হয়েছে।")

# ------------------------------------------
# ঙ) সেকেন্ড পার্টি সেটআপ (আপনার সঠিক কলাম স্কিমা অনুসারে)
# ------------------------------------------
