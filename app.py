...
# ==============================================================================
# STRUCTURE DEFINITION (THE COMPLETE APP CODE TO BE WRITTEN IN FILE)
# ==============================================================================
COMPLETE_CODE = """# ==============================================================================
...
if current_action is None:
    ...
    with tab_manual:
        ...
        c_top2.markdown(f\"<div style='padding-top:28px; font-size:13px; color:#a0a0a0;'>System Calculated Opening Vault: <b>{calculated_opening_vault:,.2f}</b></div>\", unsafe_allow_html=True)
    ...
    with tab_excel:
        ...
        if up_cash:
            try:
                ...
                if st.button(\"PROCESS SHEET IMPORT\", use_container_width=True):
                    conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
...
# ==============================================================================
# DOWNLOAD RUNTIME WRAPPER UI (MINIMALIST)
# ==============================================================================
st.set_page_config(page_title="Source Code Vault", layout="centered")

st.markdown("""
    <div style='padding: 10px 0px; border-bottom: 1px solid #2d2d2d; margin-bottom: 20px;'>
        <h4 style='margin:0; font-family:-apple-system; font-weight:600; color:#ffffff;'>M/S JABED ENTERPRISE — CORE CODE VAULT</h4>
        <small style='color:#888888;'>Generated on Runtime: 2026-06-18 | Location Reference: Bagerhat</small>
    </div>
""", unsafe_allow_html=True)

st.text_area("Review Deployment Payload (`New Final.py`)", value=COMPLETE_CODE, height=420)

st.markdown("<br>", unsafe_allow_html=True)

# Processing the raw string to byte stream for secure local downloading
code_bytes = COMPLETE_CODE.encode('utf-8')
st.download_button(
    label="📥 DOWNLOAD COMPLETE SOURCE CODE FILE",
    data=code_bytes,
    file_name="New Final.py",
    mime="text/x-python",
